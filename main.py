...
import myModels
import schemas
from myModels.responseModels import Token
...
@app.post("/api/v1/addUser",
          ...
          response_model=myModels.responseModels.Message,
          tags=["dev"]
          )
...
def get_commit_by_id(id: int, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    if verify_access_token(token, db):
        res = crud.get_commit_by_id(db, id).__dict__
        res['res'] = convert_str_to_list(res['res'])
        return res
...
def get_docs(db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    if verify_access_token(token, db):
        ...
        for index, commit in enumerate(res_in_db, 1):
            commit.id = index
        # res_in_db = convert_templete(res_in_db, convert_db_commit_to_CommitInExcel)
